...
// event handlers DONT NEED PARAMETERS
...
function init(){
   ...
   //insert the HTML code fore the first puzzle
   ...
   //add an event listener to show the solition
  document.getElementById("solve").addEventListener("click",
   function(){
      //remove the inline background color style from each cell
      if(confirm("are u sure bout dis m8?")){
...
function setupPuzzle(){
   ...
   //set the initail color of every cell in the table to gold
   for(var i = 0; i < puzzleCells.length; i++){
      puzzleCells[i].style.backgroundColor = "rgb(233, 207, 29)"

      //set the cell background in response to the mouse down event
      puzzleCells[i].onmousedown = setBackground;

      //Use the penicil image as the cursor
      puzzleCells[i].style.cursor = "url(jpf_pencil.png), pointer";
   }
   ...
   //create an event listiner to Highlieght incorrect cells
   document.getElementById("peek").addEventListener('click',
      function(){
         ...
         // Removes the hint after 0.5 seconds
         setTimeout(
            function(){
